...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status_msg.edit_text("⚙️ Sifat oshirilmoqda (Ultra Detalizatsiya)... 🛠")

        # Super 4K Sozlamalari:
        # scale=3840:2160 - Haqiqiy 4K o'lcham
        # unsharp=7:7:3.5 - Har bir detalni (yunglarigacha) o'ta o'tkir qilish
        # eq=saturation=1.8 - Ranglarni juda yorqin va realistik qilish
        cmd = [
            'ffmpeg', '-i', input_path,
            '-vf', 'scale=3840:2160:force_original_aspect_ratio=increase,crop=3840:2160,unsharp=7:7:4.0:7:7:0.5,eq=saturation=1.9:contrast=1.4',
            '-c:v', 'libx264', 
            '-preset', 'veryfast', 
            '-crf', '10',           # Eng yuqori sifat (past raqam = baland sifat)
            '-b:v', '60M',          # Maksimal tiniqlik uchun 60 Megabit bitrate
            '-pix_fmt', 'yuv420p', 
            output_path, '-y'
        ]
